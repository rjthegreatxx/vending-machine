...
public class VendingMachineService {
    ...
    public ReturnSodaDTO purchaseSoda(PurchaseSodaDTO purchaseSodaDTO) {
        ...
        BigDecimal depositAmountFromStr = new BigDecimal(purchaseSodaDTO.getDepositAmount());
        ...
        try {
            ...
            } else if(_soda.getPrice().compareTo(depositAmountFromStr) > 0){
            ...
            //purchase and log
                ...
                int numberOfQuarters = (int)(depositAmountFromStr.floatValue() / .25);
                ...
                returnSodaDTO =  new ReturnSodaDTO(_soda.getName(), 1, "Enjoy!", "0.00");
            } else if(_soda.getPrice().compareTo(depositAmountFromStr) < 0){
                Cash _cash = cashRepository.findByTypeContaining("Quarters").get(0);

                int numberOfQuartersRequiredForPurchase = (int)(_soda.getPrice().floatValue() / .25);
                int numberOfQuartersDeposited = (int)(depositAmountFromStr.floatValue() / .25);
                ...
                salesHistoryRepository.save(new SalesHistory(_soda.getName(), purchaseSodaDTO.getQuantity()));
                returnSodaDTO =  new ReturnSodaDTO(_soda.getName(), 1, "Enjoy!", String.format("%.2f", valToReturn));
            }
        } catch (Exception e) {
